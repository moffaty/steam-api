...
export class SteamAPI {
    ...
    public async GetProfileInfo(
        ...
        return await this.doRequest<Responses.ProfileResponse>(
            SteamEndPoints['user'],
            SteamEndPoints['user']['profile'],
            Versions[2],
            { steamids: steamid },
        );
    }
    ...
    public async GetOwnedGames(
        steamid: string,
    ): Promise<Responses.GamesResponse> {
        return await this.doRequest<Responses.GamesResponse>(
            SteamEndPoints['service'],
            SteamEndPoints['service']['games'],
            Versions[1],
            { steamid },
        );
    }

    public async GetUserStatsForGame(
        steamid: string,
        appid: string,
        l?: string,
    ): Promise<Responses.UserStatsResponse> {
        return await this.doRequest<Responses.UserStatsResponse>(
            SteamEndPoints['stats'],
            SteamEndPoints['stats']['gameStats'],
            Versions[2],
            { steamid, appid, l },
        );
    }
    ...
    public async GetNews(
        ...
        count: number,
        maxlength: number,
    ): Promise<Responses.NewsResponse> {
        return await this.doRequest<Responses.NewsResponse>(
            SteamEndPoints['news'],
            SteamEndPoints['news']['app'],
            Versions[2],
            { appid, count, maxlength },
        );
    }

    public async GetGlobalAchievs(
        gameid: string,
    ): Promise<Responses.GlobalAchievementsResponse> {
        return await this.doRequest<Responses.GlobalAchievementsResponse>(
            SteamEndPoints['stats'],
            SteamEndPoints['stats']['globalStats'],
            Versions[2],
            { gameid },
        );
    }

    public async GetLastGames(
        steamid: string,
    ): Promise<Responses.RecentlyGamesResponse> {
        return await this.doRequest<Responses.RecentlyGamesResponse>(
            SteamEndPoints['service'],
            SteamEndPoints['service']['recently'],
            Versions[1],
            { steamid },
        );
    }
